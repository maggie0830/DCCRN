...
def cal_si_snr_with_pit(source, estimate_source, source_lengths):
    ...
    Args:
        source: [B, C, T], B is batch size
        estimate_source: [B, C, T]
        source_lengths: [B], each item is between [0, T]  ， 每个通道VAD的长度
    """
    ...
    mean_target = torch.sum(source, dim=2, keepdim=True) / num_samples  # [B , C , 1] source每个批次每个通道的均值

    mean_estimate = torch.sum(estimate_source, dim=2, keepdim=True) / num_samples  # 预测的值 每个批次每个通道的均值
    zero_mean_target = source - mean_target  # 均值归一化
    ...
    # permutations, [C!, C]
    perms = source.new_tensor(list(permutations(range(C))), dtype=torch.long)  # [ C!,C ]，这里示例是三个通道，所以是0,1,2的所有排列组合
    # one-hot, [C!, C, C]
    index = torch.unsqueeze(perms, 2)
    # * 是取出其中的值 -> tensor[6,3] -> 6,3
    perms_one_hot = source.new_zeros((*perms.size(), C)).scatter_(2, index, 1)  # [ C!,C,C ]
    # [B, C!] <- [B, C, C] einsum [C!, C, C], SI-SNR sum of each permutation
    snr_set = torch.einsum('bij,pij->bp', [pair_wise_si_snr, perms_one_hot.float()])  # 每一种组合对应的 si-snr
...
def reorder_source(source, perms, max_snr_idx):
    ...
    Returns:
        reorder_source: [B, C, T] ， 对预测后的结果重排
    """
    B, C, *_ = source.size()
    # [B, C], permutation whose SI-SNR is max of each utterance
    # for each utterance, reorder estimate source according this permutation
    # print(perms, max_snr_idx)
